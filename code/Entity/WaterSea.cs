		...
		public void MakeTesselatedPlane( int xRes, int yRes, int size, Material material )
		{
			...
			for ( int x = 0; x <= xRes; x++ )
			{
				for ( int y = 0; y <= yRes; y++ )
				{
					...
					vertices.Add( new Vertex( new Vector3( pos.x * size, pos.y * size, 0 ), Vector3.Down, Vector3.Right, uv ) );
				}
			}
			...
			var model = Model.Create( $"TesselatedPlane-{NetworkIdent}.vmdl", material, vertices.ToArray(), indices.ToArray() );
			SetModel( model );
		}
